...
int save(Table* table){                              //внесение таблицы в файл
    ...
    for (int i = 0; i < table->msize; i++){
        Item* item = table->ks[i];
        while(item != NULL){
            fseek(fd, 0, SEEK_END);
            fwrite(&item->len_k, sizeof(int), 1, fd);

            fseek(fd, 0, SEEK_END);
            fwrite(item->key, sizeof(char), item->len_k, fd);

            fseek(fd, 0, SEEK_END);
            fwrite(&item->release, sizeof(int), 1, fd);

            fseek(fd, 0, SEEK_END);
            fwrite(&item->offset, sizeof(int), 1, fd);

            fseek(fd, 0, SEEK_END);
            fwrite(&item->len, sizeof(int), 1, fd);
...
int insert(Table *table, char* k, char *str){       //  функция добавления элемента
    ...
    item->len_k = strlen(k) + 1;
    item->next = NULL;

    int index = func_index(item->key, table->msize);

    int rel = 1;          //внесение 'release'
    Item* item_rel = table->ks[index];
    while(item_rel != NULL){
        if(strcmp(item_rel->key, item->key) == 0){
            rel = item_rel->release + 1;
            break;
        }
    ...
    item->offset = ftell(table->fd);

    item->next = table->ks[index];
    table->ks[index] = item;
    fwrite(str, sizeof(char), item->len, table->fd);
    return 0;
}
...
void delet_key_and_release(Table* table, char** k, int r){           //удаление по заданному 'Key' и 'Release'
    if(table != NULL){
        ...
        Item * item = table->ks[index]; //текущий
        Item* item2 = NULL;           //предыдущий
        while(item != NULL){
            if((item->release == r) && (strcmp(item->key, *k) == 0)){
                if(table->ks[index] == item){
                    table->ks[index] = item->next;
                    free(item->key);
                    free(item);
                    item = table->ks[index];
                }else{
...
int dialog(Table* table){
    ...
    table->msize = size;
    table->ks = (Item**) calloc (table->msize, sizeof(Item*));
    i = input_file(table);
    while(i == 0){
